...
const segmentClient = AnalyticsBrowser.load({ writeKey: SEGMENT_WRITE_KEY });

function mergeUrl(properties) {
  const href = typeof window !== 'undefined' ? window.location.href : undefined;
  if (href === undefined) return properties;
  if (properties && typeof properties === 'object' && !Array.isArray(properties)) {
    return { ...properties, url: href };
  }
  return { url: href };
}

// Wrap track() so every event includes current page URL (not added on identify/edit)
export const analytics = new Proxy(segmentClient, {
  get(target, prop, receiver) {
    const value = Reflect.get(target, prop, receiver);
    if (prop === 'track' && typeof value === 'function') {
      return function trackWithUrl(event, properties, options) {
        return value.call(target, event, mergeUrl(properties), options);
      };
    }
